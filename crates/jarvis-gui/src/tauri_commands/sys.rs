...
pub fn run_jarvis_app() -> Result<(), String> {
    ...
    if !jarvis_app_path.exists() {
        return Err(format!(
            "jarvis-app not found at: {}. Run: cargo build -p jarvis-app",
            jarvis_app_path.display()
        ));
    }

    // jarvis-app требует libvosk.dll и др. рядом с exe (копируются post_build.py из lib/windows/amd64).
    #[cfg(target_os = "windows")]
    {
        let vosk_dll = exe_dir.join("libvosk.dll");
        if !vosk_dll.exists() {
            return Err(format!(
                "Не найден {} — без него ассистент сразу завершается. \
Из корня репозитория выполните: python post_build.py \
(или cargo make в задаче с post_build), затем снова нажмите «Запустить».",
                vosk_dll.display()
            ));
        }
    }
